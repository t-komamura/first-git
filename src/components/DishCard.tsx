  ...
  return (
    <Link href={`/dishes/${dish.id}`} className="block bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow p-4">
      <div className="flex items-start gap-2">
        {rank && <span className="text-xl font-bold leading-tight shrink-0" style={{ color: '#C2410C' }}>#{rank}</span>}
      ...
      <div className="mt-2 flex items-center justify-between">
        ...
        <span className="text-xs text-gray-400">{dish.variationCount}種</span>
      </div>
    </Link>
  )
}
